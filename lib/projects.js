...
import { resolveMediaUrl, resolveMediaUrlsInText } from "./media-url";

const projectsDirectory = path.join(process.cwd(), "app", "(works)", "content", "projects");
...
function normalizeProject(slug, data = {}) {
    ...
    return {
        ...
        month: data.month ?? 1,
        monthNumber,
        coverImage: resolveMediaUrl(data.coverImage ?? fallbackCoverImage),
        ...
        tools: (data.tools ?? []).map((tool) => ({
            ...tool,
            avatar: resolveMediaUrl(tool.avatar),
        })),
...
export async function getProjectBySlug(slug) {
    ...
    return {
        ...
        content: resolveMediaUrlsInText(content),
    };
}
